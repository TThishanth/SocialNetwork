...
import 'package:SocialNetwork/pages/homePages/edit_profile.dart';
...
class _ProfileState extends State<Profile> {
  ...
  handleUnfollowUser() async {
    ...
    // delete our following users posts
    ...
    if (followingDoc.exists) {
      QuerySnapshot followingUserPostColl =
          await postsRef.doc(widget.profileId).collection('userPosts').get();
      followingUserPostColl.docs.forEach((doc) {
        timelineRef
            .doc(currentUserId)
  ...
  handleFollowUser() async {
    ...
    // Make auth user to follow others (Update their followers collection)
    followersRef
        .doc(widget.profileId)
        .collection('userFollowers')
        .doc(currentUserId)
        .set({});
    // Put THAT user on OUR following collection (update our following collection)
    followingRef
        .doc(currentUserId)
        .collection('userFollowing')
        .doc(widget.profileId)
    ...
    // Get our following users
    DocumentSnapshot followingDoc = await followingRef
        .doc(currentUserId)
        .collection('userFollowing')
        .doc(widget.profileId)
        .get();
    // store following users post in timeline
    if (followingDoc.exists) {
      QuerySnapshot followingUserPostColl =
          await postsRef.doc(widget.profileId).collection('userPosts').get();
      followingUserPostColl.docs.forEach((doc) {
